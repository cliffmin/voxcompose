...
/**
 * Optimized Ollama API client with connection pooling and efficient HTTP handling. Reuses
 * connections and maintains a connection pool for better performance.
 */
public class OllamaClient {
  ...
  /**
   * Create a new OllamaClient with specified endpoint and timeout. Uses a shared connection pool
  ...
  public OllamaClient(String endpoint, int timeoutMs) {
    ...
    this.client = SHARED_CLIENT.newBuilder().callTimeout(Duration.ofMillis(timeoutMs)).build();
  }
  ...
  public boolean isAvailable() {
    Request request =
        new Request.Builder().url(endpoint.replace("/api/generate", "/api/tags")).get().build();
  ...
  /** Shutdown the shared connection pool. Should be called when the application exits. */
